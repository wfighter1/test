...
class TaskQueue
{
    ...
    public function addTask($conferenceId, $tempConferenceId, $startTime, $endTime, $env, $callback)
    {
        ...
        $this->db->insert('task_queue', [
            ...
            'create_at' => $startTime,
            'available_at' => time()+600,//延后十分钟再开始，保证MPS已经转换完毕
            'attempt' => 0,
            'status' => 0,
        ]);
    ...
    public function getTask()
    {
        ...
        try {
            $sql = "SELECT * FROM task_queue WHERE start_at=0 AND available_at<=? AND `status`=0 AND attempt<? ORDER BY attempt,available_at LIMIT 1";
            $task = $this->db->fetchAssoc($sql, [time(), self::MAX_RETRY]);
            if ($task) {
                $retry_interval= max($task['conference_length'] * 3, self::MIN_RETRY_DELAY);//加长重试间隔时间
